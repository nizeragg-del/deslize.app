  ...
  try {
    ...
    const fullHtml = `
      ...
      <html>
        <head>
          <meta charset="utf-8">
          <!-- Tailwind CSS Play CDN: needed so all utility classes generated by the AI (w-5, h-5, rounded-2xl, inline-flex, grid, gap-*, etc.) are compiled and applied correctly in the exported PNG -->
          <script src="https://cdn.tailwindcss.com"><\/script>
          <style>
            body { margin: 0; padding: 0; background: #0A0A0F; color: white; font-family: sans-serif; }
            /* Force SVG icons to never overflow their inline context */
            svg { max-width: 100%; max-height: 100%; }
            svg.inline-block { display: inline-block !important; }
    ...
    // 'load': waits for the full page load including external scripts (Tailwind CDN) and fonts before proceeding
    await page.setContent(fullHtml, { waitUntil: 'load' })

    // Extra stabilization delay: gives Tailwind Play CDN engine time to parse the DOM and compile all utility classes
