      ...
      <header className="flex h-20 items-center justify-between border-b border-white/10 px-7">
        ...
        <div className="flex items-center gap-7">
          ...
          <button
            ...
            className={`inline-flex items-center gap-3 rounded-full border px-6 py-3 text-base font-bold transition ${
              dashboardMode
                ? 'border-purple-400/55 bg-purple-500/10 text-purple-300'
                : 'border-purple-400/55 bg-purple-500/10 text-purple-300'
            }`}
          >
              ...
              <div className="min-w-0">
                <div className="mb-16 flex items-center justify-between gap-6 rounded-3xl border border-white/10 bg-[#17181b]/90 px-6 py-5 shadow-[0_18px_50px_rgba(0,0,0,0.24)]">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <h2 className="text-lg font-bold">Sua primeira geração é gratuita</h2>
                      <p className="mt-1 text-sm text-white/55">Configure seu Brand Kit, gere seu primeiro carrossel e depois escolha o plano ideal.</p>
                    </div>
                  </div>
                  <Link href="/dashboard/planos" className="shrink-0 rounded-xl bg-purple-500 px-5 py-3 text-sm font-bold text-white shadow-[0_12px_28px_rgba(168,85,247,0.28)]">
                    Ver planos
                  </Link>
                </div>
                  ...
                  <div className="flex items-center justify-between border-t border-white/15 pt-5">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <select
                        ...
                        className="rounded-xl border border-white/12 bg-[#25262b] px-5 py-3 text-base font-semibold text-white outline-none"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-xl border border-white/12 bg-white/[0.06] px-5 py-3 text-base font-semibold text-white disabled:opacity-50"
                        type="button"
                      >
                        {generatingIdea ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-5 w-5 text-purple-300" />}
                        Gerar ideia
                      </button>
                    </div>
                    <button
                      onClick={() => createProject(true)}
                      disabled={!prompt.trim() || creating || !brandConfigured}
                      className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-purple-500 text-white shadow-[0_10px_28px_rgba(168,85,247,0.35)] disabled:opacity-45"
                ...
                <div className="mt-7 grid grid-cols-3 gap-4">
                  {suggestions.map((suggestion) => (
                    <button
                      ...
                      className="flex min-h-[70px] items-center gap-3 rounded-2xl border border-white/12 bg-white/[0.04] px-5 py-3 text-left text-sm font-semibold leading-snug text-white hover:bg-white/[0.08]"
                    >
                      <Lightbulb className="h-5 w-5 shrink-0 text-white/70" />
                      {suggestion}
                    </button>
                  ))}
                </div>
              ...
              <aside className="space-y-5">
                <div className="rounded-3xl border border-white/10 bg-[#17181b]/92 p-6">
                  ...
                  <p className="mb-6 text-sm leading-relaxed text-white/55">
                    Configure seu Brand Kit para que nossa IA crie conteúdos alinhados com sua marca.
                  </p>
...
function Sidebar({
  ...
  creating: boolean
  createProject: (openPrompt?: boolean) => void
  ...
  return (
    <aside className="flex flex-col border-r border-white/10 bg-[#101113] p-5">
      ...
      <button
        ...
        disabled={creating}
        className="mt-7 flex h-[76px] w-full items-center justify-center gap-4 rounded-xl border border-dashed border-white/25 text-lg font-bold hover:bg-white/[0.04] disabled:opacity-60"
      >
        <span className="flex h-9 w-9 items-center justify-center rounded-md bg-white/[0.08]">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-3xl font-extrabold">
              {profile?.credits || 0}<span className="text-xl text-purple-400">/{maxCredits}</span>
            </p>
          </div>
        </div>
        <div className="my-5 h-px bg-white/10" />
        <Link href="/dashboard/planos" className="flex items-center justify-between text-sm text-white/65 hover:text-white">
          Plano atual: {profile?.plan === 'free' ? 'Grátis' : profile?.plan || 'Grátis'}
      ...
      <div className="mt-7 grid gap-7 lg:grid-cols-[minmax(0,2fr)_380px]">
        <div className="rounded-[28px] border border-white/10 bg-[#17181b] p-10">
          <div className="mb-8 flex items-center justify-between gap-6">
            ...
            <button onClick={() => setDashboardMode(false)} className="shrink-0 text-lg font-semibold text-white hover:text-purple-200">
              Criar novo
            </button>
          </div>
          <div className="space-y-5">
            {projects.slice(0, 5).map((project) => (
              <Link
                ...
                className="flex items-center justify-between gap-6 rounded-[20px] bg-white/[0.06] px-7 py-6 text-xl transition hover:bg-white/[0.09]"
              >
        ...
        <div className="space-y-7">
          ...
          <div className="rounded-[28px] border border-white/10 bg-[#17181b] p-8">
            <div className="mb-6 flex items-start justify-between gap-4">
              ...
              <span className={`rounded-full px-3 py-1 text-xs font-bold ${brandConfigured ? 'bg-emerald-400/12 text-emerald-300' : 'bg-yellow-400/12 text-yellow-200'}`}>
                {brandConfigured ? 'Pronto' : 'Pendente'}
              </span>
            </div>
            <Link href="/dashboard/marca" className="inline-flex w-full items-center justify-center rounded-full bg-white px-5 py-4 text-sm font-bold text-black">
              Configurar Brand Kit
            </Link>
          </div>
        </div>
...
function StatCard({
  ...
  const colorClass = {
    purple: 'bg-purple-500/[0.18] text-purple-300',
...
function QuickLink({ href, icon, label, compact = false }: { href: string; icon: ReactNode; label: string; compact?: boolean }) {
  return (
    <Link
      href={href}
      className={`flex items-center gap-5 rounded-[18px] bg-white/[0.06] font-medium text-white/85 hover:bg-white/[0.1] ${
        compact ? 'justify-between px-5 py-4 text-base' : 'px-7 py-5 text-xl'
      }`}
    >
